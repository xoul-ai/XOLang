...
class Sampler(nn.Module):
    ...
    def forward(
        ...
        # NOTE: Removed hard-block computation and enforcement since we now use soft penalties in penalizers
        # Soft penalties are applied directly in the penalizer _apply() methods, so we don't need to:
        # 1. Compute hard block IDs (expensive: loops over all requests checking start positions)
        # 2. Sync them across TP ranks (expensive: all_reduce operations)
        # 3. Enforce them via enforce_hard_blocks (would override soft penalties anyway)
        # Previously this code computed hard_now and potentially synced it, but never used it.
        # Removing this saves significant computation on every forward pass.
        ...
        if needs_sync:
            ...
            # When using penalizers (DRY, unigram/bigram guards), they depend on output_ids being identical across ranks.
